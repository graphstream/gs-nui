...
int main(int argc, char** argv) {
	...
	for (int ite = 0; ite < maxIte; ite++) {
		...
		glm::dvec3 win (
			viewport[0] + static_cast<double>(dv(gen)),
			viewport[1] + static_cast<double>(dv(gen)),
			static_cast<double>(dv(gen)));
		
		viewport[2] = win[0] + static_cast<double>(dv(gen));
		viewport[3] = win[1] + static_cast<double>(dv(gen));
		
		glm::dvec3 unProjected = glm::unProject(win, view, proj, viewport);
		
		PRINTV(win,         fout)
		PRINTM(view,        fout)
		PRINTM(proj,        fout)
		PRINTV(viewport,    fout)
		PRINTV(unProjected, fout)
	}
